...
// Import hooks (make sure these files exist)
...
const InterviewSession = () => {
  ...
  let mediaMetrics = { posture: 0, eyeContact: 0 };
  let voiceMetrics = { clarity: 85, volume: 0 };
  ...
  try {
    const mediaHook = useMediaPipeAnalysis(videoRef, isCameraActive);
    if (mediaHook) mediaMetrics = mediaHook;
  } catch (e) {
    console.log('MediaPipe hook not ready yet');
  }

  try {
    const voiceHook = useSileroVAD();
    if (voiceHook) {
      isSpeaking = voiceHook.isSpeaking || false;
      voiceMetrics = voiceHook.voiceMetrics || { clarity: 85, volume: 0 };
    }
  } catch (e) {
    console.log('VAD hook not ready yet');
  }
  ...
  try {
    ...
    console.log('WebSocket hook not ready yet');
  }
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      ...
      if (lastMsg.type === 'interview_started') {
        ...
        setChatHistory(prev => [...prev, {
          type: 'ai',
          text: lastMsg.interviewer_response || lastMsg.feedback || 'Thanks for your response!',
                ...
                {msg.analysis && (
                  <div className="message-analysis">
                    <span className="analysis-badge" style={{ background: '#FF6B6B' }}>
                      STAR: {msg.analysis.star_method_score || 0}%
                    </span>
                    <span className="analysis-badge" style={{ background: '#4ECDC4' }}>
                      Technical: {msg.analysis.technical_score || 0}%
                    </span>
                    <span className="analysis-badge" style={{ background: '#FFD93D' }}>
                      Communication: {msg.analysis.communication_score || 0}%
                    </span>
                  </div>
                )}
              </div>
